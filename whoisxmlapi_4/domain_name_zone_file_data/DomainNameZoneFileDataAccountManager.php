...
require_once 'Net/SSH2.php';
	class DomainNameZoneFileDataAccountManager{
        ...
        public static function deleteAccountFromOrderItem($params, &$output){
            ...
            $domainNameDataNameVariations=array('_Zone_file_Domain_List', ' Zone file / Domain List');
            $error = false;
            foreach($domainNameDataNameVariations as $val){
                ...
                if($index!==false){
                    ...
                    if(DomainNameZoneFileDataAccountManager::deleteAccount($username,$end_date,$output)){
        ...
        public static function createAccountFromOrderItem($params, &$output){
			...
			foreach($domainNameDataNameVariations as $val){
				...
				if($index!==false){
					...
					$user_type=substr($item_name,$index+strlen($val), $next_index- $index - strlen($val));
					if(DomainNameZoneFileDataAccountManager::createAccount($username,$password,$user_type,$output, $active_days)){
						DomainNameZoneFileDataAccountManager::emailCustomer($customerEmail, array("username"=>$username,"password"=>$password,"user_type"=>$user_type));
						...
						$body="Order filled for $customerEmail:$EOL".
     						...
     						"-type: $user_type daily zone file / domain list $EOL".
     						"-cost: $price$EOL".
						...
						DomainNameZoneFileDataAccountManager::emailSupport(array("subject"=>$subject, "body"=>$body));
			...
			$body = "An error has occured while processing order for $customerEmail:$EOL" .
					...
					"error_detail:$EOL".
					"$error$EOL";
			$subject="Error processing $item_name for $customerEmail";		
			DomainNameZoneFileDataAccountManager::emailSupport(array("subject"=>$subject, "body"=>$body));
			return false;
		}
		...
		private static function construct_domain_data_string($user_type, $prefix){
			...
			$body="-type: all registered domains $EOL". 
		...
		public static function emailCustomer($email, $params){
			...
			$body="Dear customer,$EOL" .
				"The following is your access information for downloading daily domain name zone file data $user_type. $EOL" .
     			"-username: $username$EOL" .
     			"-password: $password$EOL" .
				// self::construct_domain_data_string($user_type, "_Zone_file_Domain_List");
			/*if($INCLUDE_NGTLDS){
				$body.="The following is your access information for downloading new GTLDs: $EOL";
				$body.=self::construct_domain_data_string($user_type, "ngtlds_Zone_file_Domain_List");
			}*/
			...
			$subject="Domain Name Zone file Data $user_type download information";
