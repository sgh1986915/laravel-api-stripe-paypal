...
<?php 
	...
	if(!$err){
		$order_types = array('wc'=>'whois_api_client',
			'cctld_wdb'=>'cctld_whois_database',
			'wdb' =>'whois_database',
			'bwl' =>'bulk_whois_lookup_service',
			'custom_wdb'=>'custom_whois_database'
		);
	
		if($order_types[$_REQUEST['order_type']]){?>
			<script type ="text/javascript">
				$(document).ready(function(){
					location.href=location.href+"#<?=$order_types[$_REQUEST['order_type']]?>";	
				});
			</script>
		<?php 	
		}
...
<form id='bwl_form' name="bwl_form" action="<?php echo $form_action?>" class="ignore_jssm">
...
<form id='wpc_form' name="wpc_form" action="<?php echo $form_action?>" class="ignore_jssm">
...
<p class="rightTxt3">
	We provide archived historic whois database in both parsed and raw format for download as databases dumps(Mysql or Mssql dump) or CSV files. Data will be delievered between 2 to 10 business days(depending on the volume).  


</p>
<form id='wdb_form' name="wdb_form" action="<?php echo $form_action?>" class="ignore_jssm">
...
<form id='cctld_wdb_form' name="cctld_wdb_form" action="<?php echo $form_action?>" class="ignore_jssm">
