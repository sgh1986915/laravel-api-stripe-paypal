...
function linkfy_report_detail($text, $report){
  ...
  $url =  build_url("$app_root/reverse-ip/report_detail.php", array('report_id'=>$hashed_report_id)) ;
...
if(!$username){
  output_json_error(array("error"=>"you must login to view your reports."));
  exit;
}
...
$SQL = sprintf("select  count(rep.report_id) from reverseip_report rep where username = '%s' and view_flag = 1", mysql_real_escape_string($username));
...
$SQL = sprintf("select  rep.* from reverseip_report rep where username = '%s' and view_flag = 1 $filter LIMIT $start , $limit", mysql_real_escape_string($username));

$result = mysql_query( $SQL ) or die("Couldn t execute query.".mysql_error() . " <br/>query:$SQL");

$i=0; 
while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
  $rep = ReverseIPReport::get_report_from_db_row($row);
   $responce->rows[$i]['id']=$rep->report_id; 
   $reportName=linkfy_report_detail($rep->getReportName(), $rep);
   $responce->rows[$i]['cell']=array($rep->created_date, $reportName, $rep->getNumResults(), $rep->getPrice());
   $i++; 
}
