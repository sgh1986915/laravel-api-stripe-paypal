<?php 
    ...
    $cart = $_SESSION['cart'];
     if(!is_object($cart)){
      $order_error = "You didn't order any Reverse IP reports. " ;
     }
  }
  ...
  if(!$order_error){	
	if(is_object($user = $_SESSION['myuser'])){
		...
		if($userAccount){
			$balance = max(0, $userAccount->reverse_ip_balance) + max(0, $userAccount->reverse_ip_monthly_balance);
			...
			if($num_items_cart > 0 && $balance > 0 && $balance  >= $num_credits_needed){
    				...
    				if(!$payment_util->fill_order($_SESSION['order_id'], 'RI')){
    					$order_error =  $payment_util->error;
    				}
					else {
						if(!$userAccount->deduct_ri_credits($num_credits_needed)){
							$order_error = $userAccount->error;
						}
						else{
							header('location:' . "$USERS_BASE_URL/thankyou_ri.php?order_type=report");
							exit();
						}
					}
	...
	if($order_error){
		include dirname(__FILE__).'/../reverse-ip-order.php';
		exit();
	}
	$url = false;
	...
	if(!strcmp($pay_choice,"pp")){
		...
		$params = array(//'cmd' => '_ext-enter', 'redirect_cmd'=>'_xclick',
			...
			'page_style' =>'whoisxmlapi',
			 'no_shipping'=>1,
			 'cancel_return'=>'http://www.whoisxmlapi.com/reverse_ip/order.php',
    ...
    //$cart->show_ordered_rows_by_type('R');
	  $params['item_name'] = "Reverse IP Reports for $order_username";
