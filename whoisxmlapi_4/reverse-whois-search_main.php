...
<?php
  ...
  function rev_whois_report_table_title(){
    global $search_type;
    // $type=($_REQUEST['search_type']==2 ? 'Current and Historic' : 'Current');
...
function setSearchType(data){
  if(data && data.search_type){
    	$('#whoisform [name=search_type]').attr('checked',data.search_type==2);
...
jQuery("#revWhoisGrid").jqGrid({
  url:BASE_URL+'whoisxmlapi_4/<?php echo $whois_url;?>', // modified
