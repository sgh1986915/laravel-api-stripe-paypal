...
require_once __DIR__. '/paypal.class.php';  // include the class file
     ...
     if ($p->validate_ipn()) {
		...
		payment_log("!ipn validated ". print_r($p->ipn_data,1));
		 ...
		 //if(in_array($p->ipn_data['business'], $ALL_PAYPAL_EMAILS)){
			...
			if(is_subscription_failed($p)){
				handle_subscription_failed($p);
		 	...
		 	if(isset($p->ipn_data['txn_type']) && ( stripos($p->ipn_data['item_name'], 'Daily Domain Name Data') !==false)){//daily domain name data
                ...
                else if($p->ipn_data['txn_type'] == 'subscr_cancel' || ($p->ipn_data['txn_type'] == 'subscr_failed')){//cancellation
                    //txn_type=subscr_cancel, item_name=Daily Domain Name Data Enterprise, subscr_date=09:56:35 Jan 13, 2014 PST
                    payment_log("subscr_cancel");
                    try{
                        $params=array(
                            'item_name'=>$item_name,
                        ...
                        if(!DomainNameDataAccountManager::deleteAccountFromOrderItem($params, $output)){
                        	...
                        	$subscr_start_date=$p->ipn_data['subscr_date'];
                        	if ($p->ipn_data['txn_type'] == 'subscr_failed'){
                        		$end_date = date("g:i A m/d/Y");
                        	}
                        	else $end_date=DomainNameDataAccountManager::calculateSubscriptionEndDateFromItemName($subscr_start_date, $item_name);
                            notifyMeOrderProcessed(array('subject'=>"plan to delete newly registered domains account on $end_date, startdate=$subscr_start_date",
                                'body'=>'params: '.print_r($params,1) . "\n"
                                    ."output error: ".print_r($output,1)
                            ));
            ...
            else if(isset($p->ipn_data['txn_type']) && stripos($p->ipn_data['item_name'], 'Reverse IP') !==false ){//REVERSE IP
                $bulk_ri_index = stripos($item_name, "Reverse IP Lookups");
                $bulk_ri_month_index = stripos($item_name, "Reverse IP Lookups/month");

                if($bulk_ri_month_index>0){
                    $occ="ly membership subscription of ";
                    $index = stripos($item_name, $occ);
                    ...
                    $quantity = substr($item_name, $index + strlen($occ), strrpos($item_name, "Reverse IP lookups")-strlen($occ)-$index-1);
                    payment_log($item_name);
                    payment_log("reverse ip membership vars: $quantity, $membership_payperiod, $accountUserName, ");
                    ...
                    if(is_numeric($quantity) && !empty($accountUserName)){
                        ...
                        $fulfillRes = reverseWhoisFillMonthlyQuantity($quantity, $accountUserName, array('membership_payperiod'=>$membership_payperiod));
                        if($fulfillRes) emailOrderProcessed($to, array('accountUserName'=>$accountUserName,
                            'orderQuantity'=>$quantity, 'item_name'=>$item_name, 'order_type'=>'reverse_ip_credits_monthly',
                            'extraBody' =>"Your account's reverse ip lookup balance will be reset to $quantity every month on day ".date('d') ."."
                        ));
                        $invoice_data=array('username'=>$accountUserName);

                    }
                }
                else if($bulk_ri_index > 0){
                    $quantity = trim(substr($item_name, 0, $bulk_ri_index));
                    $accountUserName = trim(substr($item_name, $bulk_ri_index+strlen("Reverse IP lookups for ")));
                    if(is_numeric($quantity) && !empty($accountUserName)){
                        $fulfillRes = reverseIPAddQuantity($quantity, $accountUserName);
                        payment_log("fulfill $fulfillRes");
                        if($fulfillRes) emailOrderProcessed($to, array('accountUserName'=>$accountUserName,
                            'orderQuantity'=>$quantity, 'item_name'=>$item_name, 'order_type'=>'reverse_ip_credits'));
                        $invoice_data=array('username'=>$accountUserName);
                    }
                }
                    ...
                    $order_id = $p->ipn_data['item_number'];
                    $payment_util = new payment_util();
                    if(!$payment_util->fill_order($order_id, 'RI')) $error= $payment_util->error;
                    if($error) payment_log("reverse ip report error: $error");
                    else {
                        payment_log("reverse ip report filled: ".$p->ipn_data['item_name']);
                        emailOrderProcessed($to, array("item_name"=>$item_name, "order_type"=>"reverse_ip_report"));
                    }
                    $invoice_data=array('email_to'=>$to);
                }
            }
            ...
            else if(isset($p->ipn_data['txn_type']) && ( stripos($p->ipn_data['item_name'], 'Bulk Whois Lookup') !==false )){
                payment_log("Bulk Whois Lookup filled: ".$p->ipn_data['item_name']);
                processBulkWhoisLookupOrder(array('item_name'=>$p->ipn_data['item_name'], 'email_to'=>$to));
                $invoice_data=array('email_to'=>$to);

            }
            else{
			  if(isset($p->ipn_data['txn_type']) && $p->ipn_data['txn_type']=='subscr_payment'){
				...
				$invoice_data=array('email_to'=>$to,'username'=>$accountUserName);
...
//handlers
function is_subscription_failed($p){
	if(!$p || (!($ipn_data=$p->ipn_data)))return false;
	if($ipn_data['txn_type']!='subscr_failed')return false;
	return true;
}
function handle_subscription_failed($p){
	if(!$p || (!($ipn_data=$p->ipn_data))) return;
	$payer_email=$ipn_data['payer_email'];
	$item_name=$ipn_data['item_name'];
	$payment_gross=$ipn_data['payment_gross'];
	notify_payment_failure(array('payer_email'=>$payer_email, 'item_name'=>$item_name, 'payment_gross'=>$payment_gross));
}

function notify_payment_failure($data){
	$emailer=new Email;
	$emailer->from="support@whoisxmlapi.com";
	$emailer->bcc="support@whoisxmlapi.com";
	$payer_email=$data['payer_email'];
	$subject="Your subscription payment has failed";
	$item_name=$data['item_name'];
	$payment_gross=$data['payment_gross'];
	$body="Dear customer,\n We are unable to process your payment for the subscription below:\n"
			."$item_name for $"."$payment_gross\n".
			"To avoid service interruption, please either resolve this with your payment processor(eg. paypal) or go to our website www.whoisxmlapi.com to signup for this service again.\n".
			"Please email us at support@whoisxmlapi.com if you have any questions.";
			
	$emailer->send_mail($payer_email,$subject,$body,null);
}

function test(){
	$p=array('item_name'=>'Daily Domain Name Data Enterprise', 'txn_type'=>'subscr_failed', 'payer_email'=>'topcoder1@gmail.com', 'payment_gross'=>109);
	$paypal_input=new stdClass;
	$paypal_input->ipn_data=$p;
	if(is_subscription_failed($paypal_input)){
		//print_r($paypal_input);
		handle_subscription_failed($paypal_input);
	}
}
