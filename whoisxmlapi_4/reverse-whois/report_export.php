...
function export_to_file_pdf($dest_file, $response, $report, $format){
  ...
  $title = "Reverse Whois Report";
  $header = "Search results for ".$report->render_search_terms() ."\nCompiled on ".date('m/d/Y H:i:s',$report->created_date_long);
...
function create_report_fname($report, $format){
  $search_terms= $report->search_terms[0];
  $date = date('Y_m_d', $report->created_date_long);
  //$s = "ReverseWhois" . ($report->search_type==2?"Historic":"Current") ."_$search_terms". "_$date";   
  $s = "ReverseWhois"  ."_$search_terms". "_$date";   
  return "$s.$format";
}
...
if(!$username){
  output_error(array("error"=>"you must login to view your whois reports."));
  exit;
}
...
$report = $report_util->get_report_by_id($plain_report_id);
if($report === false){
 ...
 */
   ...
   $response->header=array('Domain Name');
   $start=0;
   ...
   for($i=$start;$i<=$max;$i++){
      ...
      $response->rows[$index++]['cell']=array($domain_name); 
