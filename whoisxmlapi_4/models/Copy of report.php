...
class Report{
  ...
  public static function get_history_whois_records($options, $noncryptic=false){
   ...
   $SQL = "select * from whois_current_index where match ('$match') group by domain_name within group order by id desc LIMIT $start , $limit option max_matches = $MAX_DOMAIN_SEARCH_MATCH, cutoff = $MAX_SEARCH_CUTOFF";
   ...
   // echo $SQL;
    ...
    if(!$meta_only){
      ...
      foreach($groups as $group){
            ...
            $whois_records[] = "<a href=\"".
            		build_url($whois_record_path, array_merge(array('d'=>$domain_name_hash, 'w'=>$group['audit_updated_date'], 'search_type'=>$options['search_type']),
                                                                 array_key_pick($_REQUEST, 'term')
                                                                )
                                                            )
   		...
   		/*
      if($i > 0 && connect_to_rw_whoiscrawler_db()){  
      	$j=0; 
      	$sub_query = 'select domain_name, updated_dates from whois_aggregate where ';
      	foreach($groups as $group){
        	$sub_query .= " domain_name = '".$group['domain_name'] . "'";
        	if($j>=0 && $j<$i-1) $sub_query .=' or ';
        	$j++;
      	}
        ...
        while ( $row = mysql_fetch_assoc($sub_result) ){ 
			$updated_dates[$row['domain_name']] = $row['updated_dates'];
        	$j++;
    	}
    	...
    	for($j=0;$j<$i;$j++){
    		...
    		$row = $response->rows[$j]['cell'];
    		$domain_name = $row[0];
    		$cur_dates = $row[1];
    		$str = $updated_dates[$domain_name];
    		
    		if(trim($str)){ 
    			$dates = explode(' ',$str);
    			//print_r($dates);
    			foreach($dates as $d){
    			 $response->rows[$j]['cell'][1] .= " <a href=\"".
            		build_url($whois_record_path, array_merge(array('d'=>$domain_name, 'w'=>$d, 'search_type'=>$options['search_type']),
                                                                 array_key_pick($_REQUEST, 'term')
                                                                )
                                                            )
            			. "\" class=\"whois_record\">" . date('m/d/Y',$d) . '</a>';  
