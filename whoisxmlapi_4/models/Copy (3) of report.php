...
class Report{
  ...
  public static function get_history_whois_records($options, $noncryptic=false){
  	...
  	return self::get_whois_records(array('index'=>$WHOIS_HISTORY_INDEX), $options,$noncryptic);
  }
  ...
  */
  ...
  public static function get_current_whois_records($options, $noncryptic=false){
  	...
  	return self::get_whois_records(array('index'=>$WHOIS_CURRENT_INDEX), $options,$noncryptic);
  }
  
  public static function get_whois_records($core_options, $options, $noncryptic=false){
   ...
   //	$SQL = "select * from $index where match ('$match') group by domain_name  order by  audit_updated_date desc LIMIT $start , $limit option max_matches = $MAX_DOMAIN_SEARCH_MATCH, cutoff = $MAX_SEARCH_CUTOFF";
	$SQL = "select * from $index where match ('$match') group by domain_id  within group order by  audit_updated_date desc LIMIT $start , $limit option max_matches = $MAX_DOMAIN_SEARCH_MATCH, cutoff = $MAX_SEARCH_CUTOFF";
