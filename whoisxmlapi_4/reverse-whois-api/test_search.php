...
function check_rate_limit(){
	...
	if(RateLimit::checkRateLimit($type, $username))
		...
		output_error(RATE_LIMIT_ERR, "$username has exceeded rate limit of ". RateLimit::getRateLimitPolicy($type));
		exit;
	}
}
...
$res = '';
$options = array( 'start'=>$start,'page'=>$page,'limit'=>$limit, 'search_type'=>$search_type, 'search_terms'=>$search_terms, 'since_date'=>$since_date, 'days_back'=>$_REQUEST['days_back']);
