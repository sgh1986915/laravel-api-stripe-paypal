...
class Report{
  ...
  public static function get_history_whois_records($options, $noncryptic=false){
    ...
    $SQL = "select * from $WHOIS_HISTORY_INDEX where match ('$match')  $time_filter group by domain_name LIMIT $start , $limit option  max_matches = $MAX_DOMAIN_SEARCH_MATCH, cutoff = $MAX_SEARCH_CUTOFF";
    //echo $SQL;
  ...
  public static function get_current_whois_records($options, $noncryptic=false){
    ...
    if($since_date){
    	...
    	if(!isset($options['days_back'])){ //defaults to 18 months
    		$options['days_back']=540;
    	}
    	
    	if($options['days_back']){
    		//$since_date_unix_time = time() - $options['days_back']*3600*24;
    		$since_date_unix_time = time() - $options['days_back']*3600*48;//defaults to 2 days instead of 1, this might not be right
    		$time_filter=" and audit_updated_date>= $since_date_unix_time";
    	}
    }
    $meta_only = $options['meta_only'];
