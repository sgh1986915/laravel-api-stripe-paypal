<?php require_once __DIR__ . "/util.php";
	...
	if(!$pay_choice){
		...
		if(!$membership && !$query_quantity)
			$order_error = "You must either select the number of Reverse IP lookups you wish to purchase or pick a membership.";
	}
	if($order_error){
		// include 'bulk-reverse-ip-order.php';
		// modified
		setcookie('order_error',$order_error,time()+3600,'/');
		header('Location: ../bulk-reverse-ip-order.php');
		exit();
	}
	...
	if(!strcmp($pay_choice,"pp")){
		...
		$params = array(//'cmd' => '_ext-enter', 'redirect_cmd'=>'_xclick',
			...
			'page_style' =>'whoisxmlapi',
			 'no_shipping'=>1,
			 'return'=>'http://www.whoisxmlapi.com/thankyou_ri.php?order_type=credit'
		);
		if($membership){
			$riMonthlyQueryAmount = $membership['query_amount'];


			$payperiod = $membership['payperiod'];
			$cost = (strcasecmp($payperiod, 'month')===0? $riMembershipPrices[$riMonthlyQueryAmount] : $riMembershipPrices[$riMonthlyQueryAmount]*10);

			$params['cmd']='_xclick-subscriptions';
			$itemDescription = ucwords($payperiod) . "ly membership subscription of $riMonthlyQueryAmount Reverse IP lookups/month for $order_username";
			...
			$params['cmd'] = '_xclick';
			$params['item_name'] = "$query_quantity "  . ' Reverse IP lookups for '.$order_username;
			$params['amount'] = $PAYMENT_TEST ? 1: $riQueryPrices[$query_quantity];
